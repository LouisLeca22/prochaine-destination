...
export default function ActivitiesList({ activities }: { activities: Activity[] }) {
    ...
    const t1 = useScopedI18n("Activities")
    ...
    if (activities.length === 0)
        return <p className="h-full p-4 flex items-center justify-center self-end text-primary text-2xl">{t1("NoActivity")}</p>;
    ...
    return (
        <div className="relative">
            ...
            {selectedId && <ActivityDetail activities={activities} closeDrawer={closeDrawer} openDrawer={openDrawer} />}
        </div>
    );
}