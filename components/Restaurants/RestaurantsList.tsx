...
export default function RestaurantsList({ restaurants }: { restaurants: Restaurant[] }) {
    const locale = useCurrentLocale()

    const t1 = useScopedI18n("Restaurants")
    ...
    const { selectedId, setSelectedId } = useRestaurantFilterStore()
    ...
    if (restaurants.length === 0)
        return <p className="h-full p-4 flex items-center justify-center self-end text-primary text-2xl">{t1("NoRestaurant")}</p>;
    ...
    return (
        <div className="relative">
            ...
            {selectedId && <RestaurantDetail restaurants={restaurants} closeDrawer={closeDrawer} openDrawer={openDrawer} />}
        </div>
    );
}